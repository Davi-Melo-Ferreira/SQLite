...
# Caminho Relativo
db_path = Path('codigo_final') / 'bd_rel_1_n.db'
...
# Criação da tabela 'Perdidos' (Tabela relacionada)
cursor.execute('''
CREATE TABLE IF NOT EXISTS Clientes(
...
def inserir_pedido():
    ...
    try:
        ...
        if not cliente_existe(id_cliente):
            ...
            print('-'*70)
            # Retorne ao menu se o cliente não existir
            return
        ...
        valor_total = float(input('Digite o valotr total: '))
